...
function run(params) {
  ...
  function downloadAndProcessChapter(session, chap, volumeIndex, resultChapters, resultImages, state, storyUrl) {
    ...
    while (attempt < maxAttempts) {
      try {
        ...
        console.log("  [!] Lỗi tải chương: " + e.message);
        if (e.message.indexOf("đăng nhập") !== -1) throw e;
      }
    ...
    if (imageSources.length > 0) {
      ...
      for (let img = 0; img < imageSources.length; img++) {
        ...
        if (base64) {
          ...
          const internalPath = "images/hako_" + (volumeIndex ? "v" + volumeIndex + "_" : "") + state.imageCounter + "." + ext;
    ...
    resultChapters.push({
      id: volumeIndex ? "vol_" + volumeIndex + "_chap_" + (resultChapters.length + 1) : "chap_" + (resultChapters.length + 1),
      title: pageTitle,
      text: content,
      rawHtml: true
    });
    ...
    waitAfterHakoResponse(chapResp, randomDelay);
  }
  ...
  function buildVolumeEbook(volume) {
    ...
    for (let i = 0; i < volumeChapters.length; i++) {
      ...
      downloadAndProcessChapter(session, chap, volume.index, resultChapters, resultImages, state, storyUrl);
    }
  ...
  for (let i = 0; i < chapters.length; i++) {
    ...
    downloadAndProcessChapter(session, chap, 0, resultChapters, resultImages, state, storyUrl);
  }
